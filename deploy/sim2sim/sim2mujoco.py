...
from legged_gym.legged_gym.envs.hurricane_mid_walk.hurricane_mid_walk_config import HurricaneMidWalkRoughCfg

ROOT_DIR = os.path.dirname(__file__)

# 默认角度需要与 isaac 一致
default_dof_pos=[-0.16,0.68,1.3 ,0.16,0.68,1.3, -0.16,0.68,1.3, 0.16,0.68,1.3]
    ...
    for _ in tqdm(range(int(cfg.sim_config.sim_duration / cfg.sim_config.dt)), desc="Simulating..."):
        ...
        if 1:
            ...
            if count_lowlevel % cfg.sim_config.decimation == 0:
                ...
                policy_input = np.zeros([1, num_observations], dtype=np.float16) # 同 isaac 完整
                hist_obs_input = np.zeros([1, history_len*n_proprio], dtype=np.float16) # 同 isaac 观测buf
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_model', type=str, default='', help='Run to load from.')
    parser.add_argument('--terrain', action='store_true', default=False)
    args = parser.parse_args()

    class Sim2simCfg(HurricaneMidWalkRoughCfg):

        class sim_config:
            if args.terrain:
                mujoco_model_path = f'{ROOT_DIR}/world/world_terrain.xml'
            else:
                mujoco_model_path = f'{ROOT_DIR}/world/world.xml'
            sim_duration = 60.0
            dt = 0.001 #1Khz底层
            decimation = 20 # 50Hz
        ...
        class robot_config:
            ...
            kp_all = 3.5
            kd_all = 0.15
            kps = np.array([kp_all] * 12, dtype=np.double)
            kds = np.array([kd_all] * 12, dtype=np.double)
